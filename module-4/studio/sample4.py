...
def print_analysts(analysts: List[Analyst]):
    """打印 analysts 列表的详细信息"""
    if not analysts:
        print("没有找到 analysts")
        return
    
    print(f"\n生成的 Analysts 数量: {len(analysts)}\n")
    print("=" * 60)
...
graph.invoke({
    "topic": "How LangGrapy is a good option for AI development.",
    "max_analysts": 3
}, thread)
...
#     # "human_analyst_feedback": "please focus on the CEO or CIO of the Chinese startups"
#     "human_analyst_feedback": "approve"
# }, as_node="human_feedback")

# graph.invoke(None, thread)
graph.invoke({"human_analyst_feedback": "approve"}, thread)
state = graph.get_state(thread)
print(state.next)
analysts = state.values.get("analysts", [])
print_analysts(analysts)
